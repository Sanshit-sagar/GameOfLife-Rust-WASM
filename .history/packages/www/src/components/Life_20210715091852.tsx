...
export type PerfTime = {
  prevPerfTime: number, 
  handlePerfTimeUpdate: (updatedPerfTime: number) => void,
};

const Life: React.FC<PerfTime> = ({ prevPerfTime }: PerfTime, handlePerfTimeUpdate) => {
  ...
  const updateTime = (updatedTime: number) => {
    handlePerfTimeUpdate(updatedTime);
  }
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      updateTime(endRenderTime - startRenderTime);
    };
      ...
      <div tw="mb-4">
        <div tw="flex-col h-full w-full justify-start">
          <div tw="space-y-4 mb-4">
            <h1 tw="font-bold text-4xl text-black">Game of Life</h1>
          </div>
