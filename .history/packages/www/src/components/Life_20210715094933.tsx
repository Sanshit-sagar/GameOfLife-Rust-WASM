...
const Life: React.FC = () => {
  ...
  const [perfTimes, setPerfTimes] = useState([0]);
  ...
  useEffect(() => {
    ...
    const renderLoop = () => {
      ...
      let delta = endRenderTime - startRenderTime
      let diff = 1/delta*1000
      setPerfTimes([...perfTimes, diff]); 
    };
      ...
      <div tw="mb-4">
        <div tw="flex-col h-full w-full justify-start">
          ...
          <div tw="container mx-auto p-5 mt-5 rounded-md shadow-lg border border-black">
            ...
            <li>
              {/* <ul> latest: {counter} </ul> */}
              <ul> avg of last 100 </ul>
              <ul> min of last 100 </ul>
              <ul> max of latest 100 </ul> 
            </li>
            <p> {JSON.stringify(perfTimes.splice(perfTimes.length - 10, perfTimes.length - 1))} </p>
          </div>
        </div>
      </div>
